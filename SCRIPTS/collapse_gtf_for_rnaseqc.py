    ...
    def __init__(self, gtfpath):
        ...
        with opener as gtf:
            for row in gtf:
                ...
                if annot_type == "gene" and len(self.genes) % 1000 == 0:
                    sys.stdout.write("\rParsing GTF: {} genes processed".format(len(self.genes)))
                    sys.stdout.flush()
            sys.stdout.write("\rParsing GTF: {} genes processed\n".format(len(self.genes)))
            sys.stdout.flush()

        if len(self.genes) == 0:
            raise ValueError(
                "Parsed 0 genes from {}. Check that the input is a GTF with gene features.".format(gtfpath)
            )
...
def interval_union(intervals):
    """Return the union of all intervals in the input list."""
...
def add_transcript_attributes(attributes_string):
    """
    Add transcript attributes if they were missing.
...
def collapse_annotation(annot, transcript_gtf, collapsed_gtf, blacklist=set(), collapse_only=False, stranded=False):
    """
    Collapse transcripts into a single gene model; remove overlapping intervals.

    Options:
      collapse_only: only collapse transcripts of each gene, do not remove overlaps
      stranded: only consider genes on the same strand when removing overlaps
    """
    ...
    for g in annot.genes:
        ...
        for t in g.transcripts:
            if (
                ...
                and (("tags" not in t.attributes) or len(set(t.attributes["tags"]).intersection(exclude)) == 0)
            ):
                for e in t.exons:
    ...
    opener = open_text(transcript_gtf, "r")

    with io.open(collapsed_gtf, "w", encoding="utf-8") as output_gtf, opener as input_gtf:
        ...
        for g in annot.genes:
            if g.id in new_coord_dict:
                ...
                output_gtf.write(
                    "\t".join([g.chr, g.source, "gene", start_pos, end_pos, ".", g.strand, g.phase, attr]) + "\n"
                )
                output_gtf.write(
                    "\t".join([g.chr, g.source, "transcript", start_pos, end_pos, ".", g.strand, g.phase, attr])
                    + "\n"
                )
                if g.strand == "-":
                    new_coord_dict[g.id] = new_coord_dict[g.id][::-1]
                for k, i in enumerate(new_coord_dict[g.id], 1):
                    exon_attr = attr + ' exon_id "{}_{}"; exon_number "{}";'.format(g.id, k, k)
                    output_gtf.write(
                        "\t".join([g.chr, g.source, "exon", str(i[0]), str(i[1]), ".", g.strand, g.phase, exon_attr])
                        + "\n"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--transcript_blacklist", help="List of transcripts to exclude")
    parser.add_argument(
        "--collapse_only",
        action="store_true",
